...
class HyperConv(Module):
    def __init__(self, layers, emb_size, n_node):
        ...
        self.w_ih = Parameter(torch.Tensor(3*self.emb_size, self.emb_size))
        self.w_hh = Parameter(torch.Tensor(3*self.emb_size, self.emb_size))
        self.b_ih = Parameter(torch.Tensor(3*self.emb_size))
        self.b_hh = Parameter(torch.Tensor(3*self.emb_size))
    ...
    def get_embedding(self, adjacency, embedding):
        ...
        # embedding = torch.mm(embedding, self.weight)
    ...
    #     gi = F.linear(item_embeddings, self.w_ih, self.b_ih)
    #     gh = F.linear(embedding, self.w_hh, self.b_hh) 
    ...
    #     hy = newgate + inputgate * (embedding - newgate)
...
class Disen(Module):
    ...
    #     s_id_loss1 = self.cos_sim(self.mlp_seq1(sess_id), self.mlp_seq2(sess_id_pro))
    #     s_id_loss2 = self.cos_sim(self.mlp_seq3(sess_id), self.mlp_seq4(sess_mo))
    #     s_id_loss = torch.log1p(torch.exp(s_id_loss1)) - torch.log1p(torch.exp(s_id_loss2) + torch.exp(s_id_loss1))
    #     s_id_loss = torch.sum(s_id_loss)  # 求和以得到一个标量

    #     # Step 2: 多模态内部损失计算
    #     s_mo_loss1 = self.cos_sim(self.mlp_seq4(sess_mo), self.mlp_seq5(sess_mo_pro))
    #     s_mo_loss2 = self.cos_sim(self.mlp_seq7(sess_mo), self.mlp_seq8(sess_id))
    #     s_mo_loss = torch.log1p(torch.exp(s_mo_loss1)) - torch.log1p(torch.exp(s_mo_loss2) + torch.exp(s_mo_loss1))
    #     s_mo_loss = torch.sum(s_mo_loss)  # 求和以得到一个标量

    #     # Step 3: 结合ID和多模态的交叉损失
    #     id_mo_sim = self.cos_sim(self.mlp_seq3(sess_id), self.mlp_seq4(sess_mo))
    #     cross_modality_loss = torch.log1p(torch.exp(id_mo_sim))
    #     cross_modality_loss = torch.sum(cross_modality_loss)  # 确保也是标量

    #     # Step 4: 总损失
    #     loss_pro = s_id_loss + s_mo_loss + cross_modality_loss
    #     return -loss_pro  # 返回损失用于最小化
    ...
    def seq_pro_loss(self, sess_id, sess_id_pro, sess_mo, sess_mo_pro):
        ...
        # Step 3: 结合ID和多模态的交叉损失
        id_mo_sim = self.cos_sim(self.mlp_seq3(sess_id), self.mlp_seq4(sess_mo))
        cross_modality_loss = torch.log1p(torch.exp(id_mo_sim)) - torch.log1p(torch.exp(s_id_loss1) + torch.exp(s_mo_loss1)) # 相比增加分母 （id之间、多模态之间的对比）
        cross_modality_loss = torch.sum(cross_modality_loss)  # 确保也是标量
    ...
    def seq_cri_loss(self, item_emb, text_emb, sess_id_emb, sess_mo_emb, flag, lab):
        zeros = torch.zeros(1, self.emb_size, device=item_emb.device)  # Use the same device as item_emb
        item_emb_table = torch.cat([zeros, item_emb], 0)
        ...
        cri_id_mo = torch.logsumexp(torch.stack([id_dis, mo_dis]), dim=0) - id_dis
        cri_mo_id = torch.logsumexp(torch.stack([mo_dis, id_dis]), dim=0) - mo_dis
        
        loss_id_mo = torch.where(flag == 1, cri_id_mo + 0.5 * cri_mo_id, cri_mo_id + 0.5 * cri_id_mo)
        cri_loss = torch.sum(loss_id_mo, dim=0)
    ...
    #     cri_id_mo = (torch.log10(torch.exp(id_dis)) - torch.log10(torch.exp(mo_dis) + torch.exp(id_dis)))
    #     cri_mo_id = (torch.log10(torch.exp(mo_dis)) - torch.log10(torch.exp(mo_dis) + torch.exp(id_dis)))
        
    #     # 使用 torch.where 来处理 flag 数组
    #     loss_id_mo = torch.where(flag == 1, cri_id_mo + 0.5 * cri_mo_id, cri_mo_id + 0.5 * cri_id_mo)

    #     cri_loss = torch.sum(loss_id_mo,0)
